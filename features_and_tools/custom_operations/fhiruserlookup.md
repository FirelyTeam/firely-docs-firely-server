...
- The search could not make use of additional data that was available in the SSO claims but not directly mapped to FHIR search parameters.
...
The high-level flow when `$fhirUser-lookup` is enabled:

1. Firely Auth receives claims from the SSO / identity provider.
...
Include the plugin namespace in the `PipelineOptions` section of the {ref}`appsettings <configure_appsettings>`:
...
After enabling the plugin, confirm that `$fhirUser-lookup` appears in the `CapabilityStatement` returned by `GET [base]/metadata`. Firely Auth uses this check to decide whether to invoke the operation or fall back to a direct search.
