...
#. The order of loading knowlegde and conformance resource has been fixed. Now, the definitions stored in the administration database are loaded first and the definitions from the ZIP file are loaded last. Any custom implementations of IModelContributor are loaded after the database and before the ZIP file.
