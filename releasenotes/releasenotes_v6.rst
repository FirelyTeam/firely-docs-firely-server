...
#. Chained and reverse-chained (``_has``) searches on SQL Server now use a more efficient query plan for large, non-selective chains with no change in search results.
#. MongoDB: reverse-chained (``_has``) searches (e.g. ``Patient?_has:Coverage:beneficiary:identifier=...``) no longer scale with the number of resources of the searched type. Performance is now independent of the searched population size, matching how forward-chained searches already behave. The ``MongoDbOptions:ChainingThreshold`` setting now also applies to ``_has`` sub-queries.
