...
#. We introduced the ``$fhirUser-lookup`` operation to look up the fhirUser claim of a patient or practitioner user in Firely Auth. This operation already existed internally in Firely Server to support fhirUser lookups for Firely Auth. It is now also exposed as a public operation that can be called by custom plugins or external systems. For more information see :ref:`fhiruser_lookup`.
