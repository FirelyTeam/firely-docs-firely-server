...
#. ``Library/$evaluate`` and ``$cql`` now log a ``CouldNotMapAnyListElement`` warning when a list result held elements but none of them could be mapped to the response. The client still receives an empty list; the warning records the parameter name and the number of dropped elements, so that the missing result can be traced. See :ref:`feature_cql_result_mapping`.
...
#. A custom ``IAuthorization`` implementation no longer has every read and search refused when virtual multi-tenancy (``Vonk.Plugin.VirtualTenants``) is enabled. There, Firely Server adapts an ``IAuthorization`` that does not implement ``IAuthorizationV2``, and that adaptation refused reads, version reads, searches and history requests regardless of what ``CanRead`` returned; writes were decided correctly. Reads, searches and history are decided by ``CanRead`` again, and a custom operation is left to the operation's own authorization. See :ref:`accesscontrol_api` and :ref:`feature_multitenancy`.
#. The Bulk Data Export ``$exportstatus`` and ``$exportfilerequest`` operations and the Real World Testing operations (``$realworldtesting``, ``$realworldtestingstatus``) are now registered on every ``PipelineOptions`` branch that includes their plugin. When the plugin was included in more than one branch, these operations were only available on the first of those branches.
#. Application Insights telemetry is now flushed when Firely Server shuts down gracefully. Buffered telemetry could previously be lost when the process exited before it had been sent. See :ref:`configure_log_insights`.
