...
#. Configure the ``FhirUserLookupClaimsMapping`` if the FhirUser claim cannot be derived from the ID token
    ...
    From **Firely Auth 4.6.0** onwards, the lookup is delegated to the ``$fhirUser-lookup`` operation on Firely Server (if the server advertises support for it in its ``CapabilityStatement``), instead of executing a plain system-level search. See :ref:`fhiruserlookup` for details on enabling and configuring this operation in Firely Server.
